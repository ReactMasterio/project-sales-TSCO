...
const ResponsiveTable = ({ searchValue, filters, onCategoryChange }) => {
  ...
  const handlePaginationChange = async (current, pageSize) => {
    ...
    try {
      const response = await fetch(
        "http://192.168.4.1:3020/get-product-ids",
  ...
  const fetchInitialData = () => {
    setLoading(true);
    fetch(
      `http://192.168.4.1:3000/api/products?${qs.stringify(
    ...
    try {
      ...
      while (page <= 100) {
        ...
        const pageResponse = await fetch(
          `http://192.168.4.1:3002/api/product/${productID}/comments/?page=${page}`,
          { signal: signal }
        );
      ...
      const postResponse = await axios.post(
        "http://192.168.4.1:3020/save-stats",
